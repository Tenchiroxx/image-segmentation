...
"""This tuner is used to determine the best hyperparameters for the separable U-Net on the salinas dataset"""
...
if __name__ == "__main__":
    ...
    train_data, train_labels, val_data, val_labels, test_data, test_labels, classes, weights = load_data.load(activate_pca=True, pca_explained_variance=0.9999)
    ...
    class CNNHyperModel(HyperModel):
        ...
        def build(self, hp):
            ...
            drop = hp.Float("dropout", min_value=0., max_value=0.5, step=0.05, default=0.1)
    ...
    def tuner_evaluation(tuner, generator, val_data, val_labels, val_weights):
        ...
        tuner.search(generator, epochs= 500, batch_size=10, steps_per_epoch=6, validation_data = (val_data, val_labels, val_weights), callbacks=[EarlyStopping(monitor='val_accuracy', patience=20)])
    ...
    tuner = BayesianOptimization(hypermodel, objective='val_accuracy', max_trials=500, num_initial_points=2, seed=2, directory = "SALINAS_unet_sep_random_search", overwrite=True)
