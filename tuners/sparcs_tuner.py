...
if __name__ == "__main__":
    ...
    train_labels = tf.one_hot(train_labels, depth=7, dtype=tf.int8).numpy()
    val_labels = tf.one_hot(val_labels, depth=7, dtype=tf.int8).numpy()
    test_labels = tf.one_hot(test_labels, depth=7, dtype=tf.int8).numpy()
    ...
    def tuner_evaluation(tuner, train_generator, val_generator):
        ...
        tuner.search(train_generator, epochs= 500, steps_per_epoch=steps_per_epoch, validation_data = val_generator, validation_steps=5, callbacks=[EarlyStopping(monitor='accuracy', patience=10)])
    ...
    #tuner = RandomSearch(hypermodel, objective='accuracy', max_trials=500, seed=2, directory = "SPARCS_random_search", max_model_size=100000000, overwrite=True)
    tuner = BayesianOptimization(hypermodel, objective='accuracy', max_trials=500, num_initial_points=2, seed=2, directory = "SPARCS_random_search", max_model_size=100000000, overwrite=True)




    output_dir = Path("./output_tuner")
